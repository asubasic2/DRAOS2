...
class PregledMed extends Component {
    constructor(props) {
        super(props);
        this.state = {
            Unosi:[],
    ...
    componentDidMount(){
        ...
        fetch("/ms_proizvodnja/Vrcanje/" + this.state.KosnicaId, options).
            then((response) => response.json()).
                then((responseJson)=>{
                    ...
                    this.state.koja = "Pregled vrcanja meda za košnicu - " + this.state.KosnicaId + " -";
                    var l=[];
                    if(o == 0){
